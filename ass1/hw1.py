...
#     print("result : ", sess.run(af, feed_dict={c3: 3}))
#     print("c3 : ", sess.run(c3, feed_dict={c3: 3}))
...
def my_relu(in_value):
    ...
    """
    c3 = tf.constant(0.0)
    out_value = tf.maximum(c3,in_value)
    return out_value
# aa= my_relu(-1)
# with tf.Session() as sess:
#     print("result : ", sess.run(aa))
#     print("c3 : ", sess.run(c3, feed_dict={c3: 3}))
...
def my_perceptron(x):
    ...
    a2 = tf.Variable(tf.ones([x]) )
    result = tf.multiply(i, a2, name="result")
    out_row = tf.reduce_sum(result)
    out  = my_relu(out_row)
    return i, out
...
# 	print("input :" ,sess.run(i,feed_dict ={i:[1, 2 , 3, 4]}))
# 	print ("output : ",sess.run(out,feed_dict ={i:[1, 2 , 3, 4]}))
...
def onelayer(X, Y, layersize=10):
    ...
    x = X.get_shape()[1]
    y = layersize
    w = tf.Variable(tf.zeros([x,y],dtype=tf.float32))
    b = tf.Variable(tf.zeros([y],dtype=tf.float32 ))
...
def twolayer(X, Y, hiddensize=30, outputsize=10):
    ...
    x = int(X.get_shape()[1])
    y = hiddensize
    w1 = tf.Variable(tf.random_normal([x,y],stddev=0.01))
    b1 = tf.Variable(tf.zeros([y],dtype=tf.float32) )
    logits1 = tf.matmul(X, w1) + b1
    preds1 = tf.nn.relu(logits1)
    p = int(preds1.get_shape()[1])
    q = outputsize
    w2 =tf.Variable(tf.random_normal([p,q],stddev=0.01))
    b2 = tf.Variable(tf.zeros([q],dtype=tf.float32) )
    logits = tf.matmul(logits1, w2) + b2
    preds = tf.nn.softmax(logits)
    batch_xentropy = tf.nn.softmax_cross_entropy_with_logits_v2(labels=Y,logits=logits)
    batch_loss = tf.reduce_mean(batch_xentropy)
...
def convnet(X, Y, convlayer_sizes=[10, 10],filter_shape=[3, 3], outputsize=10, padding="same"):
    ...
    """
    conv1 = tf.layers.conv2d( inputs=X, kernel_size=filter_shape,filters=convlayer_sizes[0],padding=padding,activation=tf.nn.relu)
    ...
    a = int(conv2.get_shape()[1])
    b = int(conv2.get_shape()[2])
    c = int(conv2.get_shape()[3])
    shape=a*b*c
    full_connect = tf.reshape(conv2,[-1,shape])
    x = full_connect.get_shape()[1]
    y = outputsize
    w = tf.Variable(tf.zeros([x,y],dtype=tf.float32))
    b = tf.Variable(tf.zeros([y],dtype=tf.float32))
    logits = tf.matmul(full_connect, w) + b
    preds = tf.nn.softmax(logits)
    batch_xentropy = tf.nn.softmax_cross_entropy_with_logits_v2(labels=Y, logits=logits)
    batch_loss = tf.reduce_mean(batch_xentropy)
